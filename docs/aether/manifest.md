...
| D8 | Fonts via Google Fonts in frozen pages | Theme must be self-contained | — | AETHER implementation choice: Fontshare CDN link injected by aether.js via `loadCSS` (Cabinet Grotesk + Satoshi; `aether_heading_font`/`aether_body_font` settings keep PHANTOM font-picker contract). **License (V1.2.1):** both families are Fontshare originals under the ITF Free Font License (FFL) v2.0 — free for personal and commercial use in any media; Fontshare API delivery explicitly permitted by the license (fonts delivered from ITF servers, no self-hosting required). Loaded only when the font pickers are empty; merchant-chosen Shopify fonts skip the request. NOT a Design Pack requirement — generic pack font strategy in design-pack-contract.md §7b | Identical fonts (Cabinet Grotesk + Satoshi) |
...
| D17 | Mobile grid should collapse to 1 column (frozen) | Inline `style="grid-template-columns: repeat({{ per_row }}, 1fr)"` on `.aether-shop-grid` overrides the CSS media queries (inline wins) — mobile rendered 3 columns | Inline style removed; modifier class `aether-shop-grid--cols-{2\|3\|4}` added to section + CSS base rules (media queries now effective: ≤1024 → 2 cols, ≤576 → 1 col) | Found by Task 13 parity harness; verified 1 col @390 / 3 col @1440; budget 58,418 B |
