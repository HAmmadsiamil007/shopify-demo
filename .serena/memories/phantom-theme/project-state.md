...
### Design Pack Wave 0 T2 (2026-08-17 — COMPLETE, committed, NOT pushed)
- ed2c1a9 — AETHER skeleton assets: `assets/aether.css.liquid` (1.3 KB: `.ph-client--aether` token scope `--aether-*` consuming `settings.aether_*`, zero visual rules, z-budget 5000/9000/9050/9500/9700) + `assets/aether.js.liquid` (3.4 KB: AetherRuntime skeleton, subscribes to phantom:ready + shopify:section:load/unload, inert without `[data-section-type^="aether-"]`, controllers registry shape for Wave 1). Registry gate default-pack exception auto-expired → aether asset checks now strict, all PASS. theme-check 278 files 0 offenses.
### Design Pack Wave 0 T3 (2026-08-17 — COMPLETE, committed, NOT pushed)
- b85d376 + ecc6583 — AETHER template alternates + §8 contract: `templates/index.aether.json` (verbatim mirror of index.json base — zero visual delta at promotion), `templates/collection.aether.json` + `product.aether.json` (fresh PHANTOM-section compositions; no bases exist), `docs/design-packs/template-promotion-contract.md` at REPO ROOT (theme-local docs/ is gitignored by design — tracked docs live in root docs/). No *.phantom.json archives (created only at promotion). theme-check 281 files 0 offenses; check-registry PASS. Suffix audit: aether/phantom unreserved.
### Design Pack Wave 0 T4 (2026-08-17 — COMPLETE, committed, NOT pushed)
- 1afb187 — docs materialized at REPO ROOT docs/ (tracked convention): `docs/design-packs/registry.md` (registry walkthrough + NOVA registration procedure + §17 report format), `design-pack-contract.md` (§3 anatomy/provides/must-not/CSS/JS/settings), `conversion-contract.md` (§§9-12 pipeline + DESIGN FREEZE + data-phantom-* anchors + handoff checklist), `failure-register.md` (19 rows + enforcement map), `docs/aether/manifest.md` (AETHER skeleton: identity, Waves 1-3 component registry planned, budgets, tokens, evidence placeholders). Plan's T4 acceptance met.
- NOTE: spec §15 Wave 0 steps 1-2 still OPEN (not part of plan T1-T5): commit `frontend/frontend/` as frozen visual source of truth; delete legacy stubs (media-text.liquid, newsletter-section.liquid, phantom-dark-mode.js, effects.js, three-scenes.js).
NEXT: Wave 0 T5 — AETHER chrome alternates (sections/header-group.json, footer-group.json, popup-group.json) — NOT authorized yet. Then Waves 1-3 (AETHER commerce core) after Wave 0.
